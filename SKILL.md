...
| 数据 | 位置 | 说明 |
|------|------|------|
| 登录凭据 | `credentials.json` | Windows DPAPI 加密（绑定当前用户账户，非 Windows 回退 base64 编码） |
...
```
命令：echo '{"learn_account":"X","password":"Y","username":"Z","name":"W"}' | python scripts/login_manager.py --init --cred-stdin
输出：{"status":"ok", "fields":{"learn_account":"...", "username":"...", "name":"..."}}
```
> 使用 `--cred-stdin` 通过标准输入传入 JSON，避免密码出现在命令行参数中。
