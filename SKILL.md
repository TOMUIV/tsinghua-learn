...
metadata:
  openclaw:
    requires:
      primaryCredential: cas_password
    primaryCredential: cas_password
    credentialScope: "learn.tsinghua.edu.cn, id.tsinghua.edu.cn (CAS)"
---
...
**铁律 6（提交确认）：提交作业前必须先展示确认信息，获得用户明确同意后才能提交。**
- ❌ 禁止的行为：AI 收到文件后直接提交，不展示课程、作业标题、文件名。
- ✅ 正确的行为：AI 展示"课程：微积分(2)，作业：第四次书面作业，文件：学号_姓名.pdf，确认提交吗？"
- 用户说"确认"/"提交"/"好"之后才能执行上传。
